package com.aphidmobile.flip.subcomponents;

import static com.aphidmobile.flip.subcomponents.FlipRenderer.checkError;
...
//			reloadFirstTexture();
		...
		synchronized (LOCK) {
			if (startPosition == Position.down) {			
				...
				drawDebendsOnAngleStartUp(gl);
			}
		}
	}

	private void whenPageLies() {
	...
	private void drawDebendsOnAngleStartDown(GL10 gl){
		if (angle < 90) { 
	...
	private void drawDebendsOnAngleStartUp(GL10 gl){
		if (angle < 90) { 
		...
		if (upBitmap != null) {
			...
			frontTopCard.setCardVertices(new float[] { 0f,
					...
					upBitmap.getWidth(), upBitmap.getHeight() / 2f, 0f, // bottom
																		// right
					upBitmap.getWidth(), upBitmap.getHeight(), 0f // top right
					});
...
//			downBitmap.recycle();
	...
	public void handleTouchEventDown(boolean up, int  tempCurrentViewId, View newView){
		if (angle < 180 && angle > 0) 
			return ;
